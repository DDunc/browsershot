...
function resizeWindowToMaxHeight(driver, desiredWidth) {
  ...
  return Promise.all([getWidth(driver), getHeight(driver)])
    .then(
      function(d) {
        ...
        return driver.manage().window().setSize(desiredWidth, d[1] /*+ 100*/).then(function() {
    ...
    snap: function(config) {
      ...
      function takeScreenshotAtWidth(width, url, context) {
        return setWidth(driver(), width)
          .then(
            resizeWindowToMaxHeight.bind(null, driver(), width),
