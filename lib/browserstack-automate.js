...
module.exports = function (credentials, options) {
  ...
  return {
    ...
    snap: function (config) {
      ...
      function takeScreenshotAtWidth(width, url, count) {
        return setWidth(driver(), width)
          ...
          .then(
            function () {
              const file = filepath({ url: url, width: width, count: count });
      ...
      function takeScreenshotForUrlAtWidths(url, widths, count) {
        ...
        return navigateToUrl(url)
          ...
          .then(
            function () {
              return takeScreenshotsAtWidths(widths, urlToFilename(url), count);
            },
      ...
      return urls.reduce(
        function (curr, url, count) {
          return curr.then(
            function () {
              return takeScreenshotForUrlAtWidths(url, widths, count);
            },
